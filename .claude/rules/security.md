...
> Extends [rules/common/security.md](../../rules/common/security.md) with TimeBeam security specifics.

## Secret Management
- NEVER hardcode secrets in source code
- API keys, tokens, passwords → environment variables
- Keychain for client-side token storage
- `.env` files → `.gitignore`
- `settings.local.json` → `.gitignore`

## iOS/macOS Security
- Keychain for JWT token storage (Security.framework)
...
- `deviceId` stored in Keychain — survives launches, shared across iOS/macOS via access group
