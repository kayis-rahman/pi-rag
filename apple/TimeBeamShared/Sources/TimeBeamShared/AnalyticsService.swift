...
public struct TopWindow: Equatable {
    public let startHour: Int // 0...23
    public let endHour: Int   // 0...24 (exclusive)
    public let sessionCount: Int
}

public enum AnalyticsService {
    public static func last7DaysTotals(records: [SessionRecord], calendar: Calendar = .current) -> [DailyTotal] {
        let now = Date()
    ...
    public static func productiveStreak(records: [SessionRecord], calendar: Calendar = .current) -> Int {
        let now = Date()
        let startOfToday = calendar.startOfDay(for: now)
    ...
    public static func topProductiveWindow(records: [SessionRecord], windowHours: Int = 2, calendar: Calendar = .current) -> TopWindow? {
