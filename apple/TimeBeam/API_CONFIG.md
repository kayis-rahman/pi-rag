...
TimeBeam iOS/macOS apps connect to the backend using an environment variable configuration.
...
2. **Info.plist** - Build variable substitution
   - TimeBeam/Info.plist:6 uses `$(API_BASE_URL)` placeholder

3. **Swift Fallback Code** - Environment variable support
   - TimeBeamApp.swift:47
...
2. Select "TimeBeam iOS" or "TimeBeam" (macOS) target
