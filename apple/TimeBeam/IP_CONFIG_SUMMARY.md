...
**File:** `apple/TimeBeam/TimeBeam/Info.plist`
...
**File:** `apple/TimeBeam/TimeBeam.xcodeproj/project.pbxproj`
...
**Files Updated:**
- `TimeBeam/TimeBeamApp.swift:47` - AnalyticsApiClient fallback
- `TimeBeam/Application/Services/TaskService.swift:19` - ApiClient fallback
- `TimeBeam/Presentation/Views/iOS/AnalyticsView.swift` - Multiple instances (223, 256, 283)

All now fallback to: `http://192.168.0.173:8080` if environment variable not set

### 4. Documentation
**New File:** `apple/TimeBeam/API_CONFIG.md`
...
2. Run iOS app:
   - In Xcode, select "TimeBeam iOS" scheme
...
2. Update build settings (4 locations):
   ...
   sed -i '' 's|API_BASE_URL = ".*|API_BASE_URL = "http://NEW_IP:8080"|' \
     TimeBeam.xcodeproj/project.pbxproj
   ```

3. Rebuild:
   ```bash
   xcodebuild clean build -scheme "TimeBeam iOS" \
...
2. Select target (TimeBeam iOS or TimeBeam)
...
| File | Change |
|------|---------|
| `TimeBeam/Info.plist` | Added $(API_BASE_URL) placeholder |
| `TimeBeam.xcodeproj/project.pbxproj` | Added API_BASE_URL to 4 build configs |
| `TimeBeam/TimeBeamApp.swift` | Updated fallback URL |
| `TimeBeam/Application/Services/TaskService.swift` | Updated fallback URL |
| `TimeBeam/Presentation/Views/iOS/AnalyticsView.swift` | Updated 3 instances |
| `API_CONFIG.md` | New configuration guide |

## Git Status

```
M TimeBeam.xcodeproj/project.pbxproj
M TimeBeam/Info.plist
M TimeBeam/TimeBeamApp.swift
M TimeBeam/Application/Services/TaskService.swift
M TimeBeam/Presentation/Views/iOS/AnalyticsView.swift
?? API_CONFIG.md
```
